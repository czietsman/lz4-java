...
public class LZ4Test extends AbstractLZ4Test {
  ...
  public void testUncompressWorstCase() {
    for (LZ4FastDecompressor decompressor : Instances.FAST_DECOMPRESSORS) {
  ...
  public void testUncompressSafeWorstCase() {
    for (LZ4SafeDecompressor decompressor : Instances.SAFE_DECOMPRESSORS) {
      testUncompressWorstCase(decompressor);
    }
  }
  ...
  public void testRoundTrip(byte[] data, int off, int len) {
    for (LZ4Factory lz4 : Collections.singletonList(LZ4Factory.INSTANCE)) {
  ...
  public void testWriteToReadOnlyBuffer() {
    for (LZ4Compressor compressor : Instances.COMPRESSORS) {
    ...
    for (LZ4SafeDecompressor decompressor : Instances.SAFE_DECOMPRESSORS) {
      ByteBuffer in = Tester.BYTE_BUFFER.copyOf(Instances.COMPRESSORS[0].compress(new byte[] {2, 3}));
      ByteBuffer out = Tester.BYTE_BUFFER.allocate(100).asReadOnlyBuffer();
      try {
