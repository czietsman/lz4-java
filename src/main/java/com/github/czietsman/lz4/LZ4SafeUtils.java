...
enum LZ4SafeUtils {
    ...
    static int hash(byte[] buf, int i) {
        return LZ4Utils.hash(Utils.readInt(buf, i));
    }

    static int hash64k(byte[] buf, int i) {
        return LZ4Utils.hash64k(Utils.readInt(buf, i));
    }
    ...
    static int lastLiterals(byte[] src, int sOff, int srcLen, byte[] dest, int dOff, int destEnd) {
        ...
        if (runLen >= LZ4Constants.RUN_MASK) {
            dest[dOff++] = (byte) (LZ4Constants.RUN_MASK << LZ4Constants.ML_BITS);
            dOff = writeLen(runLen - LZ4Constants.RUN_MASK, dest, dOff);
        } else {
            dest[dOff++] = (byte) (runLen << LZ4Constants.ML_BITS);
        }
        // copy literals
