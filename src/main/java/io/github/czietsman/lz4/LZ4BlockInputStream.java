...
import io.github.czietsman.xxhash.StreamingXXHash32;
import io.github.czietsman.xxhash.XXHash32;
import io.github.czietsman.xxhash.XXHashFactory;
...
public final class LZ4BlockInputStream extends FilterInputStream {
    ...
    public LZ4BlockInputStream(InputStream in, LZ4FastDecompressor decompressor) {
        this(in, decompressor, XXHashFactory.INSTANCE.newStreamingHash32(LZ4BlockOutputStream.DEFAULT_SEED).asChecksum());
    }
    ...
    private void refill() throws IOException {
        readFully(compressedBuffer, LZ4BlockOutputStream.HEADER_LENGTH);
        for (int i = 0; i < LZ4BlockOutputStream.MAGIC_LENGTH; ++i) {
            if (compressedBuffer[i] != LZ4BlockOutputStream.MAGIC[i]) {
                throw new IOException("Stream is corrupted");
            }
        }
        final int token = compressedBuffer[LZ4BlockOutputStream.MAGIC_LENGTH] & 0xFF;
        final int compressionMethod = token & 0xF0;
        final int compressionLevel = LZ4BlockOutputStream.COMPRESSION_LEVEL_BASE + (token & 0x0F);
        if (compressionMethod != LZ4BlockOutputStream.COMPRESSION_METHOD_RAW && compressionMethod != LZ4BlockOutputStream.COMPRESSION_METHOD_LZ4) {
            throw new IOException("Stream is corrupted");
        }
        final int compressedLen = Utils.readIntLE(compressedBuffer, LZ4BlockOutputStream.MAGIC_LENGTH + 1);
        originalLen = Utils.readIntLE(compressedBuffer, LZ4BlockOutputStream.MAGIC_LENGTH + 5);
        final int check = Utils.readIntLE(compressedBuffer, LZ4BlockOutputStream.MAGIC_LENGTH + 9);
        ...
        if (originalLen > 1 << compressionLevel
                ...
                || (originalLen != 0 && compressedLen == 0)
                || (compressionMethod == LZ4BlockOutputStream.COMPRESSION_METHOD_RAW && originalLen != compressedLen)) {
            throw new IOException("Stream is corrupted");
        }
        ...
        switch (compressionMethod) {
            case LZ4BlockOutputStream.COMPRESSION_METHOD_RAW:
